    ...
    <section class="wrapper site-min-height">
        ...
        <section class="no-print col-md-8">
            <header class="panel-heading">
                ...
                <div class="panel-body no-print pull-right">
                    <a href="finance/pharmacy/sellNowSingle?id=<?php echo $patient->id; ?>&type=gen">
                        <div class="btn-group">
                            <button id="" class="btn btn-xs green">
...
// Initialize totals
$total_gross = 0;
$total_deposit_running = 0;

// Calculate total gross from all payments
foreach ($payments as $payment) {
    $total_gross += floatval($payment->gross_total);
}
?>
        ...
        <table class="table table-striped table-hover table-bordered" id="editable-samples">
            <thead>
                <tr>
                    ...
                    <th class=""><?php echo 'Balance Due';//lang('deposit_type'); ?></th>
                    <th class=""><?php echo 'Credit Note';//lang('deposit_type'); ?></th>
            ...
            // Reset running totals for table display
            $running_gross = 0;
            $running_deposit = 0;
            ?>
            ...
            foreach ($dattt as $key => $value) {
                // Process payments for this date
                foreach ($payments as $payment) {
                    if ($payment->date == $value) {
                        $running_gross += floatval($payment->gross_total);
                        $amount_received = !empty($payment->amount_received) ? array_sum(array_map('floatval', array_filter(explode(',', $payment->amount_received), 'is_numeric'))) : 0;
                        $running_deposit += $amount_received;
                        $balance_due = $running_gross - $running_deposit;
                        ?>
                        <tr class="">
                            <td><?php echo date('d-m-y', $payment->date); ?></td>
                            <td><?php echo $payment->id; ?></td>
                            <td><font color="green"><?php echo number_format($payment->gross_total, 0); ?></font></td>
                            <td><font color="orange"><?php echo number_format($amount_received, 0); ?></font></td>
                            <td><?php echo $payment->deposit_type; ?></td>
                            <td><font color="red"><?php echo number_format($balance_due, 0); ?></font></td>
                            <td></td>
                            <td class="no-print">
                                <a class="btn-xs invoicebutton" title="<?php echo lang('invoice'); ?>" style="color: #fff; width: 25%;" href="finance/pharmacy/invoice?id=<?php echo $payment->id; ?>"><i class="fa fa-file-invoice"></i></a>
                                <?php if ($this->ion_auth->in_group(array('admin', 'Accountant'))) { ?>
                                    <a class="btn-xs btn-info delete_button" title="<?php echo lang('delete'); ?>" style="width: 25%;" href="finance/pharmacy/delete?id=<?php echo $payment->id; ?>" onclick="return confirm('Are you sure you want to delete this item?');"><i class="fa fa-trash"></i></a>
                                <?php } ?>
                            </td>
                        </tr>
                        <?php
                ...
                // Process deposits for this date
                foreach ($deposits as $deposit) {
                    if ($deposit->date == $value && !empty($deposit->deposited_amount) && empty($deposit->amount_received_id)) {
                        $running_deposit += floatval($deposit->deposited_amount);
                        $balance_due = $running_gross - $running_deposit;
                        ?>
                        <tr class="">
                            ...
                            <td><font color="orange"><?php echo number_format($deposit->deposited_amount, 0); ?></font></td>
                            ...
                            <td class="no-print">
                                <?php if ($this->ion_auth->in_group(array('admin', 'Accountant'))) { ?>
                                  <!--  <button type="button" class="btn-xs btn-info editbutton" title="<?php echo lang('edit'); ?>" style="width: 25%;" data-toggle="modal" data-id="<?php echo $deposit->id; ?>"><i class="fa fa-edit"></i></button>-->
                                      ...
                                      <a class="btn-xs invoicebutton" title="<?php echo lang('invoice'); ?>" style="color: #fff; width: 25%;" href="finance/pharmacy/invoice?id=<?php echo $payment->id; ?>&deposit_ref=<?php echo $deposit->id; ?>&bal_due=<?php echo $balance_due;?>"><i class="fa fa-file-invoice"></i></a>
                ...
                <section class="m_t">
                    <div class="panel-body profile">
                        <div class="task-thumb-details">
                            <?php echo lang('patient'); ?> <?php echo lang('name'); ?>: <h1><a href="#"><?php echo $patient->first_name.' '.  $patient->last_name; ?></a></h1> <br>
                            ...
                            <div class="row">
                                ...
                                <div class="col-xs-8">
                                    <div class="degree">
                                        ...
                                        <?php echo number_format($total_payable_bill = $total_bill, 0); ?>
                                    </div>
                                </div>
                            </div>
                        </div>
...
foreach ($deposits as $deposit) {
    if (!empty($deposit->deposited_amount) && empty($deposit->amount_received_id)) {
        $total_deposit += floatval($deposit->deposited_amount);
    }
}
...
// Add up extra deposits not linked to amount_received
foreach ($deposits as $deposit) {
    if (!empty($deposit->deposited_amount) && empty($deposit->amount_received_id)) {
        $total_deposit += floatval($deposit->deposited_amount);
    }
...
// Show due amount
echo number_format($total_payable_bill - $total_deposit, 0);

$total_dueSet = $total_payable_bill - $total_deposit;
?>
                                    </div>
                                </div>
                            </div>
                ...
                <form role="form" id="editDepositform" action="finance/deposit" class="clearfix" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="deposited_amount" id="exampleInputEmail1" value='' placeholder="">
                                                ...
                                                <small id="due_error" style="color:red; display:none;">
    You can't put more than patient's due balance (<?php echo number_format($total_dueSet, 0); ?>)
</small>
                    </div>
...
-->
                    ...
                    <div class="form-group  payment  right-six col-md-12">
                        <button type="submit" name="Pay Now" id="submit1" class="btn btn-info row pull-right"> <?php echo lang('submit'); ?></button>
                    </div>  
                 <!--   <div class="form-group cardsubmit  right-six col-md-12 hidden">
                            ...
                            foreach ($dattt as $key => $value) {
                                foreach ($payments as $payment) {
                                    if ($payment->date == $value) {
										...
										$r = array_sum(explode(',', $payment->amount_received));
										echo $g - $r;
                        ...
                        <div class="row">
                            <div class="col-lg-8 invoice-block pull-right total_section">
                                <ul class="unstyled amounts"> 
                                    <li><strong><?php echo lang('grand_total'); ?> : </strong><?php echo $settings->currency; ?> <?php echo $total_payable_bill; ?></li>
<li><strong><?php echo lang('amount_received'); ?> : </strong><?php echo $settings->currency; ?> <?php echo $total_deposit; ?></li>
<li><strong><?php echo lang('amount_to_be_paid'); ?> : </strong><?php echo $settings->currency; ?> <?php echo $total_payable_bill - $total_deposit; ?></li>
                                ...
                                $(".editbutton").click(function (e) {
                                    ...
                                    $.ajax({
                                        ...
                                        if (response.deposit.deposit_type != 'Card') {
                                            ...
                                            $('#editDepositform').find('[name="gross_total"]').val(response.deposit.gross_total).end()
                                            $('#editDepositform').find('[name="amount_due"]').val(response.deposit.amount_due).end()
...
document.addEventListener('DOMContentLoaded', function() {
    const depositInput = document.querySelector("input[name='deposited_amount']");
    if (depositInput) {
        depositInput.addEventListener("keypress", function (e) {
            if (e.key === "-" || e.key === "e") { // prevent minus and exponent
                e.preventDefault();
            }
        });
    }
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        let maxDue = <?php echo $total_dueSet; ?>;
        let amountInput = document.getElementById("exampleInputEmail1");
        let errorText = document.getElementById("due_error");
        let submitBtn = document.getElementById("submit1");

        amountInput.addEventListener("input", function () {
            let entered = parseFloat(amountInput.value) || 0;
