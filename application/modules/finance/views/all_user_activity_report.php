                    ...
                    <table class="table table-striped table-hover table-bordered" id="editable-samples">
                        ...
                        <?php } ?>
                            ...
                            <tr class="">
                                ...
                                <td><?php echo $settings->currency; ?><?php
                                    $total_nurse = array();
                                    $ot_total_nurse = array();

                                    $nurse_ion_user_id = $nurse->ion_user_id;
                                    foreach ($payments as $payment1) {
                                        if ($payment1->user == $nurse_ion_user_id) {
                                            $total_nurse[] = $payment1->gross_total;
                                        }
                                    }
                                    foreach ($ot_payments as $ot_payment1) {
                                        if ($ot_payment1->user == $nurse_ion_user_id) {
                                            $ot_total_nurse[] = $ot_payment1->gross_total;
                                        }
                                    }

                                    $total_nurse = array_sum($total_nurse);
                                    if (empty($total_nurse)) {
                                        $total_nurse = 0;
                                    }

                                    $ot_total_nurse = array_sum($ot_total_nurse);
                                    if (empty($ot_total_nurse)) {
                                        $ot_total_nurse = 0;
                                    }

                                    echo $bill_total_nurse = $total_nurse + $ot_total_nurse;
                                    ?>
                                </td>
                                <td><?php echo $settings->currency; ?><?php
                                    $deposit_total_nurse = array();
                                    foreach ($deposits as $deposit) {
                                        if ($deposit->user == $nurse_ion_user_id) {
                                            $deposit_total_nurse[] = $deposit->deposited_amount;
                                        }
                                    }

                                    $deposit_total_nurse = array_sum($deposit_total_nurse);
                                    if (empty($deposit_total_nurse)) {
                                        $deposit_total_nurse = 0;
                                    }
                                    echo $deposit_total_nurse;
                                    ?>
                                </td>
                                <td>
                                    <?php echo $bill_total_nurse - $deposit_total_nurse; ?>
                                </td>
                                <td class="no-print">
                                    <a class="btn btn-info btn-xs btn_width add_payment_button" style="width: 100px;" href="finance/allUserActivityReport?user=<?php echo $nurse_ion_user_id; ?>"><i class="fa fa-info"></i> Details</a>
                                </td>
                            </tr>
                        <?php } ?>
                        </tbody>
