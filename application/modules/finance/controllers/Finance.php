...
class Finance extends MX_Controller {
    ...
    function __construct() {
        parent::__construct();
        $this->load->model('logs/logs_model', 'logs_model'); // Load with alias
        $this->logs_model = $this->logs_model; // Assign to property for easy access
...
//        $this->load->model('finance/pharmacy_model');
      ...
      //  $this->load->module('paypal');
        ...
        if (!$this->ion_auth->in_group(array('admin', 'Accountant', 'Receptionist', 'Nurse', 'Laboratorist', 'Doctor','Pharmacist'))) {
    ...
    public function addPaymentView() {
        ...
        $data['discount_type2'] = $this->pharmacy_model->getDiscountType();
    ...
    public function addPayment() {
		...
		$item_selected = array_merge((array)$item_selected_cat, (array)$item_selected_med);
        ...
        if (empty($p_email)) {
            $p_email = $p_name . '-' . rand(1, 1000) . '-' . $p_name . '-' . rand(1, 1000) . '@ispecs.com';
        }
        if (!empty($p_name)) {
        ...
        if (empty($d_email)) {
            $d_email = $d_name . '-' . rand(1, 1000) . '-' . $d_name . '-' . rand(1, 1000) . '@ispecs.com';
        }
        if (!empty($d_name)) {
...
*/
		...
		$r2 = $this->finance_model->getPaymentByLastId($payment->id);
...
// Adding New Patient
		...
		//	$amount_due = $gross_total - $flat_discount;
            ...
            if (empty($id)) {
                ...
                //end
				$amount_rec_add = array_sum(explode(',', $amount_received_add));
				$amount_due = $gross_total - $amount_rec_add;
                ...
                /* //if ($deposit_type == 'Cardss') {
                    ...
                    if ($gateway == 'PayPal') {
                        ...
                        $charge = \Stripe\Charge::create(array(
                                    "amount" => $amount_received_add * 100,
                                    "currency" => "usd",
                                    "source" => $token
                        ));
                        ...
                        redirect("payu/check1?deposited_amount=" . "$amount_received_add" . '&payment_id=' . $inserted_id);
                    } else {
                ...
                //} else { */
                    ...
                    $data_payment = array('amount_received' => $amount_received, 'deposit_type' => $deposit_type, 'amount_due' => $amount_due);
                ...
                $deposit_edit_amount0 = $this->input->post('deposit_edit_amount');
                $deposit_edit_id0 = $this->input->post('deposit_edit_id');
				
		$deposit_edit_amount = implode(',',$deposit_edit_amount0);
		$deposit_edit_id = implode(',',$deposit_edit_id0);
		
		$deposit_edit_amount_ex = explode(" ",$deposit_edit_amount);
		$deposit_edit_amount_add = array_sum($deposit_edit_amount_ex);		
		
		$deposit_edit_id_ex = explode(" ",$deposit_edit_id);
		$deposit_edit_id_add = array_sum($deposit_edit_id_ex);			
				
				
                if (!empty($deposit_edit_amount_add)) {
                    $deposited_edit = array_combine($deposit_edit_id_add, $deposit_edit_amount_add);
				...
				//$amount_due2 = $gross_total - $amount_received_add;
...
// Validating Description Field
    ...
    function patientPaymentHistory() {
        ...
        if (!empty($date_from)) {
			...
			$data['deposits2'] = $this->finance_model->getDepositByPatientId($patient);
            $data['gateway'] = $this->finance_model->getGatewayByName($data['settings']->payment_gateway);
        } else {
...
// Validating Deposited Amount Field
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data = array('patient' => $patient,
                ...
                'amount_due' => $payment_details->gross_total - $deposited_amount,
                'user' => $user
            );
            if (empty($id)) {
               /* if ($deposit_type == 'Cardss') {
...
// } 
    ...
    function AllUserActivityReport() {
        ...
        if (!empty($user)) {
			...
			if ($user_group == '6') {
                $data['user'] = $this->nurse_model->getNurseByIonUserId($user);
            }
            $data['settings'] = $this->settings_model->getSettings();
            $hour = 0;
            $TODAY_ON = $this->input->get('today');
    ...
    function getPayment() {
        ...
        if (!empty($data['payments'])) {
            $output = array(
                ...
                "recordsTotal" => $this->db->get('pharmacy_payment')->num_rows(),
                "recordsFiltered" => $this->db->get('pharmacy_payment')->num_rows(),
                "data" => $info
            );
        } else {
            $output = array(
	...
	//	$dateset 	= strtotime($date);
		...
		$week_start = strtotime("sunday");
		$week_end = strtotime("Saturday");
        ...
        //$last_minute = mktime(23, 59, 59, $month, date("t", $first_minute), $year);
        //$first_minute = mktime(0, 0, 0, $month, 1, $year);
        $first_minute = $week_start;
        $last_minute = $week_end;
