...
class Pharmacy_model extends CI_model {
    ...
    function insertItemPayment($data1) {
        $this->db->insert('pharmacy_item_payment', $data1);
    }    
    ...
    function getItemPayment() {
        $this->db->order_by('id', 'desc');
        $query = $this->db->get('pharmacy_item_payment');
        return $query->result();
    }
    ...
    function getItemPaymentById($id) {
        $this->db->where('id', $id);
        $query = $this->db->get('pharmacy_item_payment');
        return $query->row();
    }
    ...
    function updateItemPayment($id, $data) {
        $this->db->where('id', $id);
        $this->db->update('pharmacy_item_payment', $data);
    }
    ...
    function getCartSubtotalByUserid($userid) {   	
        $this->db->select_sum('subtotal');
    	$this->db->from('cart');
    	$this->db->where('user_id', $userid);
    	$query = $this->db->get();
...
public function getPatientName($patient_id) {
    $query = $this->db->select('first_name, middle_name, last_name')
                      ...
                      ->get();
    $patient = $query->row();
    return $patient ? trim($patient->first_name . ' ' . $patient->middle_name . ' ' . $patient->last_name) : 'Unknown';
}
