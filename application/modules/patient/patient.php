    ...
    public function addNew() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $config = array(
                ...
                'upload_path' => "./uploads/patient/",
                'allowed_types' => "gif|jpg|png|jpeg|pdf|doc|docx",
                'overwrite' => False,
                'max_size' => "100480000", // Can be set to particular file size , here it is 2 MB(2048 Kb)
                'max_height' => "1768",
                'max_width' => "2024"
            );
            ...
            if ($this->upload->do_upload('img_url')) {
                ...
                $img_url = "uploads/patient/" . $path['file_name'];
                $data = array();
                $data = array(
...
// Validating Deposited Amount Field
    ...
    function addPatientMaterial() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $config = array(
                'file_name' => $new_file_name,
                'upload_path' => "./uploads/patient/documents/",
            ...
            if ($this->upload->do_upload('img_url')) {
                $path = $this->upload->data();
                $img_url = "uploads/patient/documents/" . $path['file_name'];
                $data = array();
                $data = array(
                    'date' => $date,
                    'title' => $title,
    ...
    function getNurseNotes() {
        ...
        foreach ($data['nurse_notes'] as $nurse_notes) {
            ...
            if ($this->ion_auth->in_group(array('admin', 'Doctor'))) {
                $options2 = '<a class="btn btn-info btn-xs btn_width delete_button" title="' . lang('delete') . '" href="patient/deleteNurseNotes?id=' . $nurse_notes->id . '&redirect=nurse_notes" onclick="return confirm(\'Are you sure you want to delete this item?\');"><i class="fa fa-trash-o"></i></a>';
            ...
            if (!empty($nurse_notes->patient_id)) {
                ...
                if (!empty($patient_info)) {
                    ...
                    $patient_details = $nurse_notes->patient_name . '</br>' . $nurse_notes->patient_address . '</br>' . $nurse_notes->patient_phone . '</br>';
                }
            } else {
                $patient_details = '';
            }
    ...
    function getDoctorNotes() {
        ...
        foreach ($data['doctor_notes'] as $doctor_notes) {
            ...
            if ($this->ion_auth->in_group(array('admin', 'Accountant', 'Receptionist', 'Laboratorist', 'Nurse', 'Doctor'))) {
                $options2 = '<a class="btn btn-info btn-xs btn_width delete_button" title="' . lang('delete') . '" href="patient/deleteDoctorNotes?id=' . $doctor_notes->id . '&redirect=doctor_notes" onclick="return confirm(\'Are you sure you want to delete this item?\');"><i class="fa fa-trash-o"></i></a>';
            ...
            if (!empty($doctor_notes->patient_id)) {
                ...
                if (!empty($patient_info)) {
                    ...
                    $patient_details = $doctor_notes->patient_name . '</br>' . $doctor_notes->patient_address . '</br>' . $doctor_notes->patient_phone . '</br>';
                }
            } else {
                $patient_details = '';
            }
