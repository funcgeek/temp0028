                    ...
                    <table class="table table-striped table-hover table-bordered" id="editable-sample">
                        ...
                        <?php foreach ($notices as $notice) { ?>
                            <tr class="">
								...
								<td>
								<a id="viewNotice" class="btn btn-info btn_width btn-xs" data-toggle="modal" href="#myModalview"  data-id="<?php echo $notice->id; ?>">
									<i class="fa fa-plus-circle"> </i>View Notice </a>
                                <?php if ($this->ion_auth->in_group(array('admin'))) { ?>
                  ...
                  <div class="modal-content "> 
                       ...
                       <div class="modal-header"> 
                            ...
                            <h4 class="modal-title">
                            	<i class="glyphicon glyphicon-user"></i> <?php echo "View Notices";//lang('add_case'); ?>
                            </h4> 
                       </div> 
                       <div class="modal-body"> 
...
$(document).ready(function(){
	...
	$(document).on('click', '#viewNotice', function(e){	
		...
		$('#viewnotice-content').html(''); // leave it blank before ajax call
		$('#modal-loader').show();      // load ajax loader
		
		$.ajax({
			url: './db/getnotice.php',
			type: 'POST',
			data: 'id='+uid,
			dataType: 'html'
		})
		.done(function(data){
			console.log(data);	
			$('#viewnotice-content').html('');    
			$('#viewnotice-content').html(data); // load response 
			$('#modal-loader').hide();		  // hide ajax loader	
		})
		.fail(function(){
			$('#viewnotice-content').html('<i class="glyphicon glyphicon-info-sign"></i> Something went wrong, Please try again...');
			$('#modal-loader').hide();
		});
