...
class Lab extends MX_Controller {
  ...
  /*      if ($this->ion_auth->in_group(array('admin','Patient'))) {
   ...
   function dashboard() {
	   $id = $this->input->get('id');
        $data = array();
        $data['frame_registrations'] = $this->lab_model->getFrameRegistration();
        $data['frame_items'] = $this->lab_model->getFrameItems();
        $data['settings'] = $this->settings_model->getSettings();
        $data['frame_types'] = $this->lab_model->getFrameTypes();
        $data['frame_processing'] = $this->lab_model->getFrameProcessing();
        $data['report_log'] = $this->lab_model->getReportLog();
  ...
  function registration() {
	   $id = $this->input->get('id');
		...
		$this->load->view('home/dashboard', $data); // just the header file
        $this->load->view('lab/registration', $data);
        $this->load->view('home/footer');
    }
...
// Adding New Patient
    ...
    function editLab() {
        if ($this->ion_auth->in_group(array('admin', 'Doctor', 'Laboratorist', 'Receptionist', 'Patient'))) {
            $data = array();
            $data['settings'] = $this->settings_model->getSettings();
            $data['categories'] = $this->lab_model->getLabCategory();
            $data['patients'] = $this->patient_model->getPatient();
