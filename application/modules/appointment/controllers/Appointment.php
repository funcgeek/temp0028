...
class Appointment extends MX_Controller {
    ...
    //Fteching Appointment Bookings from Ispecs Appeal Website 
      public function website() {
        ...
        $data['settings'] = $this->settings_model->getSettings();
        $this->load->view('home/dashboard', $data); // just the header file
        $this->load->view('website_bookings', $data);
        $this->load->view('home/footer'); // just the header file
    }
    ...
    public function addNew() {
        ...
        if (empty($p_email)) {
            $p_email = $pf_name . '-' . rand(1, 1000) . '-' . $pl_name . '-' . rand(1, 1000) . '@ispecs.com';
        }
        if (empty($password)) {
            $password = '12345';
        }
        ...
        if ($patient == 'add_new') {
            $this->form_validation->set_rules('pf_name', 'Patient First Name', 'trim|required|min_length[1]|max_length[100]|xss_clean');
        ...
        $this->form_validation->set_rules('comments', 'Comments', 'trim|min_length[1]|max_length[1000]|xss_clean');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($patient == 'add_new') {
                ...
                $data_p = array(
					...
					'location' => 'Overton Plaza',					
                    'how_added' => 'from_appointment'
                );
                $username = $this->input->post('pf_name');
                // Adding New Patient
	...
	function getAppointmentByJasonByDoctor() {
        ...
        foreach ($query as $entry) {
            ...
            $info = '<br/>' . lang('status') . ': ' . $appointment_status . '<br>' . lang('patient') . ': ' . $patient_name . '<br/>' . $patient_first_name . '<br/>'. $patient_last_name . '<br/>' . lang('phone') . ': ' . $patient_mobile . '<br/> Doctor: ' . $doctor . '<br/>' . lang('remarks') . ': ' . $entry->remarks. '<br/>' . lang('comments') . ': ' . $entry->comments;
	...
	function getAppointmentByJason() {
        ...
        foreach ($query as $entry) {
            ...
            $info = '<br/>' . lang('status') . ': ' . $entry->status . '<br>' . lang('patient') . ': ' . $patient_name . '<br/>'. $patient_first_name . '<br/>'. $patient_last_name . '<br/>' . lang('phone') . ': ' . $patient_mobile . '<br/> Doctor: ' . $doctor . '<br/>' . lang('remarks') . ': ' . $entry->remarks. '<br/>' . lang('comments') . ': ' . $entry->comments;
            if ($entry->status == 'Pending Confirmation') {
                //  $color = '#098098';
                $color = 'yellowgreen';
            }
    ...
    function getAppointment() {
        ...
        foreach ($data['appointments'] as $appointment) {
            ...
            $options3 = '<a class="btn green" title="' . lang('history') . '" style="color: #fff;" href="appointment/medicalHistory?id=' . $appointment->patient . '"><i class="fa fa-stethoscope"></i> ' . lang('history') . '</a>';
	   ...
	   function getAppoinmentList() {
        ...
        foreach ($data['appointments'] as $appointment) {
            ...
            if (!empty($patientdetails)) {
                $patientname = ' <a type="button" class="history" data-toggle = "modal" data-id="' . $appointment->patient . '"> ' . $patientdetails->name . $patientdetails->first_name. $patientdetails->last_name . '</a>';
            } else {
                $patientname = ' <a type="button" class="history" data-toggle = "modal" data-id="' . $appointment->patient . '"> ' . $appointment->patientname . $appointment->patient_first_name .$appointment->patient_last_name . '</a>';
            }
    ...
    function getPendingAppoinmentList() {
        ...
        foreach ($data['appointments'] as $appointment) {
            ...
            $patientdetails = $this->patient_model->getPatientById($appointment->patient);
            if (!empty($patientdetails)) {
                $patientname = ' <a type="button" class="history" data-toggle = "modal" data-id="' . $appointment->patient . '"> ' . $patientdetails->name . $patientdetails->first_name . $patientdetails->first_name . '</a>';
            } else {
                $patientname = ' <a type="button" class="history" data-toggle = "modal" data-id="' . $appointment->patient . '"> ' . $appointment->patientname . $appointment->patient_first_name .$appointment->patient_last_name . '</a>';
            }
            $doctordetails = $this->doctor_model->getDoctorById($appointment->doctor);
