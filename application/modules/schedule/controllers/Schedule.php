...
function timeSchedule() {
    // Try to get the doctor ID from GET param
    $doctor = $this->input->get('doctor');

    // If GET param is not provided, fallback to logged-in doctor
    if (empty($doctor) && $this->ion_auth->in_group(array('Doctor'))) {
    ...
    if (empty($doctor)) {
        show_error('Doctor ID not provided or could not be resolved.');
    }
    ...
    $this->load->view('timeschedule', $data);   // main view
