...
class SimpleEmailHandler {
    ...
    private function getEmailConfig() {
        try {
            $stmt = $this->pdo->prepare("SELECT * FROM email_config LIMIT 1");
            $stmt->execute();
            return $stmt->fetch();
        } catch (PDOException $e) {
    ...
    private function handleRequest() {
        ...
        if ($action === 'send_email' && $_SERVER['REQUEST_METHOD'] === 'POST') {
            $this->sendEmail();
        } elseif ($action === 'search_patients' && $_SERVER['REQUEST_METHOD'] === 'GET') {
            $this->searchPatients();
        } elseif ($action === 'get_sent_messages' && $_SERVER['REQUEST_METHOD'] === 'GET') {
            $this->getSentMessages();
        } else {
            $this->sendJsonResponse(false, 'Invalid action or method');
        }
    }
    
private function searchPatients() {
    try {
        $query = $this->sanitizeInput($_GET['query'] ?? '');
        $query = trim($query);
        if ($query === '') {
            $this->sendJsonResponse(false, 'Query is required');
            return;
        }

        // Escape wildcard chars in user input, then add %...%
        $qEsc = str_replace(['%', '_'], ['\%', '\_'], $query);
        $like = '%' . $qEsc . '%';
        ...
        $this->sendJsonResponse(true, 'Patients found', ['patients' => $patients]);
    } catch (PDOException $e) {
        error_log("Search patients error: " . $e->getMessage());
        $this->sendJsonResponse(false, 'Error searching patients');
    }
}
    ...
    private function getSentMessages() {
        try {
            $stmt = $this->pdo->prepare("
                SELECT recipient_email, recipient_name, subject, status, sent_at 
                FROM NewEMRemail_log 
            ...
            $this->sendJsonResponse(true, 'Logs retrieved', ['logs' => $logs]);
            
        } catch (PDOException $e) {
            error_log("Get sent messages error: " . $e->getMessage());
            $this->sendJsonResponse(false, 'Error retrieving sent messages');
        }
    }
    ...
    public function sendEmail() {
        try {
            ...
            if (!$config) {
                $this->sendJsonResponse(false, 'Email configuration not found');
                return;
            }
    ...
    private function getRecipients($send_type) {
        ...
        try {
            switch ($send_type) {
                ...
                case 'single_patient':
                    $patient_id = intval($_POST['patient'] ?? 0);
                    if ($patient_id > 0) {
                ...
                case 'manual':
                    ...
                    foreach ($manual_recipient as $email) {
